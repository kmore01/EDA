...
// Funcion que calcula la distancia maxima para encontrar tesoro desde G[i][j]
int bfs(const grafo &G, int i, int j) {
    int nodos = G.size(), distMax = -1;
    ...
    while (not Q.empty()) {
        ...
        visit[x][y] = true;
        if (G[x][y] == 't') {
            if (dist > distMax or distMax == -1) distMax = dist;
        }
...
int main () {
    ...
    if (ans != -1) cout << "distancia maxima: " << ans << endl;
